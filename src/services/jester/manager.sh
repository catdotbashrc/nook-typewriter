...
SERVICE_FUNCTIONS="$SCRIPT_DIR/../../services/system/service-functions.sh"

# Try multiple paths for service functions
if [ -f "$SERVICE_FUNCTIONS" ]; then
    . "$SERVICE_FUNCTIONS"
elif [ -f "/src/services/system/service-functions.sh" ]; then
    . "/src/services/system/service-functions.sh"
else
    echo "Error: Cannot find service-functions.sh" >&2
    echo "Searched:" >&2
    echo "  - $SERVICE_FUNCTIONS" >&2
    echo "  - /src/services/system/service-functions.sh" >&2
    exit 1
fi
