...
if [ -f "/runtime/2-application/jesteros/tracker.sh" ]; then
    TRACKER_IMPL="/runtime/2-application/jesteros/tracker.sh"
elif [ -f "$(dirname "$0")/tracker.sh" ]; then
    TRACKER_IMPL="$(dirname "$0")/tracker.sh"
else
    echo "Error: Tracker implementation not found" >&2
    echo "Searched:" >&2
    echo "  - /runtime/2-application/jesteros/tracker.sh" >&2
