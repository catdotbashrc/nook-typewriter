...
source "$UTILITIES_DIR/config/paths.sh"

# Use configured paths
TEST_DIR="$JESTEROS_TEST"
SCRIPTS_DIR="$JESTEROS_UTILITIES"
PROJECT_ROOT="$JESTEROS_PROJECT_ROOT"
...
check_requirements() {
    ...
    if [[ ! -f "$TEST_DIR/framework.sh" ]]; then
        echo -e "${RED}Test framework not found: $TEST_DIR/framework.sh${NC}"
        exit 1
    fi
